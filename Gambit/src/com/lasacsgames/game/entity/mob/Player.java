...
public class Player extends Mob {
	...
	public void spawnRandomly() {
		while (collision(0, 0)) {
			// location.x = random.nextInt(level.getWidth()) * 16 + 2;
			// location.y = random.nextInt(level.getHeight()) * 16 + 2;
		}
	}

	/*
	 * public boolean collision(double xa, double ya) { boolean b = false; for
	 * (int i = 0; i < 4; i++) { int x = (i / 2); int y = (i % 2); if
	 * (getLevel().getTile((((int) (location.x + xa - 1 + x)) >> 4), (((int)
	 * (location.y + ya - 1 + y) >> 4))).solid()) b = true; } return b; }
	 */
}
